...
class EmployeeTbl {
  ...
  async create(dbConn) {
    ...
    // also need to create the "managers" table view (ie, virtual table)
    // it simplifies some of the other queries and is a usefule view of managers
    try {
      const sql = `CREATE OR REPLACE VIEW managers AS
                   SELECT
                     r.title,
                     e.id AS employee_id,
                     CONCAT(
